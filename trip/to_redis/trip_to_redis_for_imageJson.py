...
logger.add("./logs/trip-to-redis.log",mode="w")
M_HOST = "localhost"  # 地址
M_PORT = 5002  # 端口
M_USER = "admin"  # 用户名
...
filter_col =  client['trip_image_json']['details']
...
for i in cursor:
    ...
    isIn = filter_col.find_one({'id': i['id']}, {'_id':1})
    if isIn ==None:
        item = dict()
        ...
        logger.success(i["url"]+ "  " +str(item["data"]["hotelId"]))
        index += 1
        data = pickle.dumps(item)
        # 添加到商品爬虫redis_key指定的list
        redis.rpush(REDIS_KEY, data)
    else:
        logger.info(f"{i['id']} 已采集")
logger.success(f"trip{index} 条接口数据")
client.close()
