...
index =0
col = client['trip_before2']['seed_hotels']  # 查找泰国
cursor = col.find({"districtName": {"$regex": "Malaysia"}}, no_cursor_timeout=True, batch_size=5)
...
col = client['trip_before2']['seed_hotels2']  # 查找泰国
cursor = col.find({"districtName": {"$regex": "Malaysia"}}, no_cursor_timeout=True, batch_size=5)
for i in cursor:
    item = dict()
    item['url'] = "https://my.trip.com" + i['url'] + "?locale=ms_my&curr=MYR"
