...
index =0
col = client['trip_before2']['seed_hotels']  # 查找印尼
cursor = col.find({"districtName": {"$regex": "Indonesia"}}, no_cursor_timeout=True, batch_size=5)
for i in cursor:
    item = dict()
...
col = client['trip_before2']['seed_hotels2']  # 查找印尼
cursor = col.find({"districtName": {"$regex": "Indonesia"}}, no_cursor_timeout=True, batch_size=5)
for i in cursor:
    item = dict()
    item['url'] = "https://id.trip.com" + i['url'] + "?curr=IDR&locale=id_id"
