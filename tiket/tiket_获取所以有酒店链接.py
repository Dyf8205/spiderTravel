...
class Tiket():
    ...
    def parse_sitemap_locs(self,xml_text):
        ...
        url_list2 =[]
        for url in url_list:
            if "homes-pdp" in url or "hotel-pdp" in url:  #获取所有的  酒店  民宿
                url_list2.append(url.strip())
        return url_list2


    def deatail_sitemap_locs(self,xml_text):
        l = re.findall('href="(https://www.+?)"', xml_text)
        for http in l:
            data = {}
...
if __name__ == '__main__':
    Tiket("tiket").run("https://www.tiket.com/sitemap/en-us/index.xml.gz") # 从sitmap获取所有的详细链接
