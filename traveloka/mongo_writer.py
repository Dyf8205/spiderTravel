...
# 配置
REDIS_RESULT_KEY = 'traveloka_result'
...
def writer(writer_id):
    ...
    # 初始化集合
    collist = mongo_client["traveloka"].list_collection_names()
    if "details" not in collist:
        col = mongo_client["traveloka"]["details"]
        col.create_index("uuid", unique=True)
    if "error_urls" not in collist:
        col = mongo_client["traveloka"]["error_urls"]
        col.create_index("uuid", unique=True)

    success_col = mongo_client["traveloka"]["details"]
    error_col = mongo_client["traveloka"]["error_urls"]
        ...
        while True:
            ...
            try:
                if False == data["success"]:
                    ...
                    logger.info(f"Writer {writer_id}: 写入错误记录 {data['uuid']}")
                else:
