...
class AwsWaf:
    def __init__(self, goku_props: str,
        ...
        self.session = requests.Session(impersonate="chrome",proxies=ip)
    ...
    def extract(html: str):
        host =  re.findall(r'https://([a-f0-9]+\.edge\.sdk\.awswaf\.com/[a-f0-9]+/[a-f0-9]+/)challenge', html)[0]
        return None, host

    def get_inputs(self):
        print(self.endpoint)
        res = self.session.get(f"https://{self.endpoint}/inputs?client=browser",timeout=30).json()
        return res
