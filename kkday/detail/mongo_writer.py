...
WRITER_NUM = 1  # 写入进程数量
logger.add("./logs/kkday_to_mongo_writer.log", mode='w')
...
def writer(writer_id):
    ...
    if "details" not in collist:
        col = mongo_client["kkday"]["base_details"]
        col.create_index("id", unique=True)
    if "error_urls" not in collist:
        col = mongo_client["kkday"]["base_error_urls"]
        col.create_index("id", unique=True)

    success_col = mongo_client["kkday"]["base_details"]
    error_col = mongo_client["kkday"]["base_error_urls"]
