...
# 配置
REDIS_RESULT_KEY = 'kkday_result_api'
...
WRITER_NUM = 1  # 写入进程数量
logger.add("./logs/kkday_writer_api.log", mode='w')
...
def writer(writer_id):
    ...
    # 初始化集合
    collist = mongo_client["kkday"].list_collection_names()
    if "details" not in collist:
        col = mongo_client["kkday"]["image_detail"]
        col.create_index("id", unique=True)
    if "error_urls" not in collist:
        col = mongo_client["kkday"]["base_error_urls_image"]
        col.create_index("id", unique=True)

    success_col = mongo_client["kkday"]["image_detail"]
    error_col = mongo_client["kkday"]["base_error_urls_image"]
        ...
        while True:
            ...
            try:
                if False == data["success"]:
                    ...
                    logger.success(f"Writer {writer_id}: 写入成功 {data['id']}")
