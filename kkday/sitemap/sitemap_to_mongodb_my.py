...
logger.add("./logs/error_my.log", level="ERROR")
# 成功日志 - 只记录 SUCCESS 级别
logger.add("./logs/success_my.log", filter=lambda record: record["level"].name == "SUCCESS")
import re
...
class Kkday():
    ...
    def getWithcurl(self, url):
        ...
        while True:
            try:
                resp = session.get(url, headers=HEADERS, proxies=get_random_proxy(), timeout=20, verify=False)
